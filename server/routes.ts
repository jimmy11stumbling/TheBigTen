...
import { buildSystemPrompt } from "./services/system-prompt";
import { getPlatformDatabase } from "@shared/platform-databases";
import { registerPlatformConnectionsRoutes } from "./routes/platform-connections"; // Import the new route
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/blueprint/generate", async (req, res) => {
    ...
    // Register analytics routes
    registerAnalyticsRoutes(app);
     // Register platform connections routes
    registerPlatformConnectionsRoutes(app);
...
import { analytics } from "./services/analytics";
import { registerAnalyticsRoutes } from "./routes/analytics";
import { registerPlatformConnectionsRoutes } from "./routes/platform-connections";
import { buildSystemPrompt } from "./services/system-prompt";
import { getPlatformDatabase } from "@shared/platform-databases";

// Register routes for the express app, including blueprint generation, data retrieval, and analytics.
export async function registerRoutes(app: Express): Promise<Server> {
  // Blueprint generation endpoint with SSE
  app.post("/api/blueprint/generate", async (req, res) => {
    let headersSent = false;
    ...
    // Register platform connection routes
    registerPlatformConnectionsRoutes(app);
...
// Registers routes for the express app, including blueprint generation, data retrieval, analytics, and platform connections.
export async function registerRoutes(app: Express): Promise<Server> {
  // Blueprint generation endpoint with SSE
  app.post("/api/blueprint/generate", async (req, res) => {
    let headersSent = false;
